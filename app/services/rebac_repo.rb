class RebacRepo
  ...
  def write(subject:, id:, relation:, actor:, actor_id:, actor_rel: nil, reason: nil)
    ...
    now = Time.current
    result = @conn.exec_query(<<~SQL, "rel_tuples.write", [@tenant_id, subject, id, relation, actor, actor_id, actor_rel, now, now])
      INSERT INTO rel_tuples (tenant_id, subject, subject_id, relation, actor, actor_id, actor_rel, created_at, updated_at)
      VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9)
      ON CONFLICT (tenant_id, subject, subject_id, relation, actor, actor_id, COALESCE(actor_rel, '')) DO NOTHING
      RETURNING id
    SQL
  ...
  def delete(subject:, id:, relation:, actor:, actor_id:, actor_rel: nil, reason: nil)
    result = @conn.exec_delete(<<~SQL, "rel_tuples.delete", [@tenant_id, subject, id, relation, actor, actor_id, actor_rel])
      ...
      WHERE tenant_id=$1 AND subject=$2 AND subject_id=$3 AND relation=$4
  ...
  def parents_of(subject:, id:)
    @conn.exec_query(<<~SQL, "rel_tuples.parents", [@tenant_id, subject, id]).to_a
      ...
      FROM rel_tuples
      WHERE tenant_id=$1 AND subject=$2 AND subject_id=$3 AND relation='parent'
    SQL
  end
  ...
  def expand_group_members(group_type:, group_id:)
    @conn.exec_query(<<~SQL, "rel_tuples.group_closure", [@tenant_id, group_type, group_id]).to_a
      WITH RECURSIVE m(subj_type, id) AS (
        ...
        FROM rel_tuples
        WHERE tenant_id=$1 AND subject=$2 AND subject_id=$3 AND relation='member'
      UNION
        SELECT rt.actor, rt.actor_id
        FROM rel_tuples rt
        JOIN m ON rt.tenant_id=$1 AND rt.subject = m.subj_type AND rt.subject_id = m.id
        WHERE rt.relation='member'
      )
      SELECT DISTINCT subj_type AS actor, id AS actor_id FROM m WHERE subj_type='user'
    SQL
  end
end
